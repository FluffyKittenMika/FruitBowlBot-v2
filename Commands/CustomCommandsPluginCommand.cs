...
namespace FruitBowlBot_v2.Commands
{
    class CustomCommandsPluginCommand : IPluginCommand
    {
        ...
        public List<CCommand> CustomCommands = new();
            ...
            try
            {
                        ...
                        if (msg.IsModerator)
                        {
                            if (string.Equals(msg.Arguments[0], "add", StringComparison.OrdinalIgnoreCase))
                            {
                                if (msg.Arguments.Count >= 3)
                                {
                                    ...
                                    CCommand cmd = new(newCommand, response, msg.Channel);
                ...
                foreach (var item in CustomCommands)
                {
                    if (string.Equals(item.Command, msg.Command, StringComparison.OrdinalIgnoreCase) && item.Channel == msg.Channel)
                    {
                        ...
                        {
                            message = message.Replace($"{{arg{i}}}", msg.Arguments[i]);
                        }
                        if (message.Contains("{arg"))
                        {
                            char[] c = { '{', 'a', 'r', 'g' };
                            message = message.Split(c).First();
                        }
        ...
        private void Save()
        {
            ...
            using (StreamWriter w = new(memoryPath))
            {
        ...
        private void Load()
        {
            try
            {
                ...
                using (StreamReader r = new(memoryPath))
                {
                    string line;
